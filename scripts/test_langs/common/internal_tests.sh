...
run_internal_test() {
    ...
    case "$test_name" in
        ...
        verify_delta_count)
            ...
            local delta_info=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/crs/deltas'" 2>/dev/null)
        ...
        verify_history_limit)
            ...
            local history_info=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/crs/history'" 2>/dev/null)
        ...
        verify_pagerank_convergence)
            ...
            local stats_response=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/graph/stats'" 2>/dev/null || echo "")
            if [ -z "$stats_response" ] || echo "$stats_response" | jq -e '.error' >/dev/null 2>&1; then
                echo -e "  ${YELLOW}Building graph first...${NC}"
                ssh_cmd "curl -s -X POST -H 'Content-Type: application/json' -d '{\"project_root\":\"/home/aleutiandevops/trace_test/AleutianOrchestrator\"}' 'http://localhost:12217/v1/trace/init'" 2>/dev/null
                sleep 2
            fi

            # Trigger PageRank by calling find_important via agent
            echo -e "  ${BLUE}Triggering PageRank via find_important...${NC}"
            local agent_response=$(ssh_cmd "curl -s -X POST -H 'Content-Type: application/json' -d '{\"query\":\"What are the top 3 most important functions?\"}' 'http://localhost:12217/v1/trace/agent/run'" 2>/dev/null)
            sleep 3
        ...
        verify_implements_edges)
            ...
            local graph_stats=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/graph/stats'" 2>/dev/null)
        ...
        verify_debug_crs_endpoint)
            ...
            local crs_response=$(ssh_cmd "curl -s -w '%{http_code}' 'http://localhost:12217/v1/trace/agent/debug/crs'" 2>/dev/null || echo "")
            local http_code=""
            local body=""
        ...
        verify_debug_history_endpoint)
            ...
            local history_response=$(ssh_cmd "curl -s -w '%{http_code}' 'http://localhost:12217/v1/trace/agent/debug/history'" 2>/dev/null || echo "")
            local http_code=""
            local body=""
        ...
        verify_nodes_by_name_index)
            ...
            if [ -z "$stats_response" ] || echo "$stats_response" | jq -e '.error' >/dev/null 2>&1; then
                echo -e "  ${YELLOW}Building graph first...${NC}"
                local init_response=$(ssh_cmd "curl -s -X POST -H 'Content-Type: application/json' -d '{\"project_root\":\"/home/aleutiandevops/trace_test/AleutianOrchestrator\"}' 'http://localhost:12217/v1/trace/init'" 2>/dev/null)
                sleep 2
                stats_response=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/graph/stats'" 2>/dev/null || echo "")
            fi
        ...
        verify_cache_stats_endpoint)
            ...
            local init_resp=$(ssh_cmd "curl -s -X POST -H 'Content-Type: application/json' -d '{\"project_root\":\"/home/aleutiandevops/trace_test/AleutianOrchestrator\"}' 'http://localhost:12217/v1/trace/init'" 2>/dev/null || echo "")
            ...
            for func_name in "CodeAnalysisRequest" "NewClient" "ParseAPIMessage" "WriteDataToGCS" "FetchPromptFromGCS" "DistillerRequest"; do
                local callers_resp=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/callers?graph_id=$graph_id&function=$func_name'" 2>/dev/null || echo "")
            ...
            local cache_response=$(ssh_cmd "curl -s -w '%{http_code}' 'http://localhost:12217/v1/trace/debug/cache'" 2>/dev/null || echo "")
            local http_code=""
            local body=""
        ...
        verify_cache_invalidation)
            ...
            local before_stats=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/cache'" 2>/dev/null || echo "{}")
            ...
            ssh_cmd "curl -s -X POST -H 'Content-Type: application/json' -d '{\"project_root\":\"/home/aleutiandevops/trace_test/AleutianOrchestrator\", \"force_rebuild\": true}' 'http://localhost:12217/v1/trace/init'" 2>/dev/null
            sleep 2

            # Check cache stats after rebuild
            local after_stats=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/cache'" 2>/dev/null || echo "{}")
        ...
        verify_community_modularity)
            ...
            local community_stats=$(ssh_cmd "curl -s 'http://localhost:12217/v1/trace/debug/graph/stats'" 2>/dev/null || echo "{}")
