...
func NewGeminiClient() (*GeminiClient, error) {
	...
	if model == "" {
		...
		slog.Info("GEMINI_MODEL not set, defaulting to gemini-2.5-flash")
	}
...
type geminiPart struct {
	...
	// ThoughtSignature is an opaque token from Gemini 3 models.
	// Must be echoed back on function call parts in subsequent requests.
...
func (g *GeminiClient) Chat(ctx context.Context, messages []agenttypes.Message, params GenerationParams) (string, error) {
	model := g.model
...
func (g *GeminiClient) ChatWithTools(ctx context.Context, messages []ChatMessage,
	...
	// CB-73a: Gemini 3+ models require thoughtSignature on functionCall parts.
	// Tool calls without a signature (e.g., router-originated) must be
	// formatted as text to avoid 400 errors on Gemini 3+ while keeping
	// functionCall format for Gemini 2.5 and earlier.
	requiresThoughtSig := geminiModelRequiresThoughtSignature(model)

	// Build a set of tool call IDs that will be rendered as text (no thought signature)
	// so we can also render their corresponding tool results as text.
	textToolCallIDs := make(map[string]bool)
	if requiresThoughtSig {
		for _, msg := range messages {
			if msg.Role == "assistant" && len(msg.ToolCalls) > 0 {
				for _, tc := range msg.ToolCalls {
					if tc.ThoughtSignature == "" {
						textToolCallIDs[tc.ID] = true
					}
				}
			}
		}
	...
	for _, msg := range messages {
		...
		case msg.Role == "tool" && msg.ToolName != "":
			// CB-73a: If this tool result corresponds to a text-formatted tool call
			// (no thought signature on Gemini 3+), render as user text instead.
			if textToolCallIDs[msg.ToolCallID] {
				req.Contents = append(req.Contents, geminiContent{
					Role: "user",
					Parts: []geminiPart{{Text: fmt.Sprintf("[Tool result from %s]\n%s",
		...
		case msg.Role == "assistant" && len(msg.ToolCalls) > 0:
			// CB-73a: Check if these tool calls have thought signatures.
			// On Gemini 3+, tool calls without signatures must be text.
			if requiresThoughtSig && msg.ToolCalls[0].ThoughtSignature == "" {
				// Render as text model turn instead of functionCall parts
				...
				if textContent == "" {
					...
					textContent = fmt.Sprintf("I used tools: %s",
						strings.Join(names, ", "))
				}
				req.Contents = append(req.Contents, geminiContent{
					Role:  "model",
...
func (g *GeminiClient) ChatStream(ctx context.Context, messages []agenttypes.Message,
...
func (g *GeminiClient) buildRequest(messages []agenttypes.Message, params GenerationParams) geminiRequest {
	req := geminiRequest{}
...
// geminiModelRequiresThoughtSignature returns true for Gemini 3+ models
// that require thoughtSignature on functionCall parts.
//
// Description:
//
//	Gemini 3 series models (gemini-3-*, gemini-3.1-*) return thought
//	signatures with function calls and require them to be echoed back.
//	Gemini 2.x and earlier do not use thought signatures.
//
// Inputs:
//   - model: The model name string.
//
// Outputs:
//   - bool: True if the model requires thought signatures.
//
// Thread Safety: This function is safe for concurrent use.
func geminiModelRequiresThoughtSignature(model string) bool {
