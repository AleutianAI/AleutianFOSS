...
// TestValidateExtractedPathParams tests D3b param extractor post-validation.
func TestValidateExtractedPathParams(t *testing.T) {
	tests := []struct {
		name        string
		from        string
		to          string
		...
		// Rule 1: Destination is substring of source
		{
			name:        "to is substring of from — Context.Bind/Bind",
			from:        "Context.Bind",
			to:          "Bind",
			query:       "Show the call chain from Context.Bind to data validation",
			wantFrom:    "Context.Bind",
			wantTo:      "dataValidation",
			wantChanged: true,
		},
		{
			name:        "to is substring of from — Engine.ServeHTTP/ServeHTTP",
			from:        "Engine.ServeHTTP",
			to:          "ServeHTTP",
			query:       "Find the path from Engine.ServeHTTP to request handling",
			wantFrom:    "Engine.ServeHTTP",
			wantTo:      "requestHandling",
			wantChanged: true,
		},
		{
			name:        "to is substring of from — case insensitive",
			from:        "WriteBatch.Set",
			to:          "set",
			query:       "Show the call chain from WriteBatch.Set to disk persistence",
			wantFrom:    "WriteBatch.Set",
			wantTo:      "diskPersistence",
			wantChanged: true,
		},

		// Rule 2: Same symbol
		{
			name:        "same symbol — identical",
			from:        "parseConfig",
			to:          "parseConfig",
			query:       "Find path from parseConfig to error handling",
			wantFrom:    "parseConfig",
			wantTo:      "errorHandling",
			wantChanged: true,
		},

		// Rule 3: Short generic name
		{
			name:        "short generic to — 3 chars",
			from:        "read_csv",
			to:          "csv",
			query:       "Show the call chain from read_csv to DataFrame construction",
			wantFrom:    "read_csv",
			wantTo:      "dataframeConstruction",
			wantChanged: true,
		},

		// No change cases
		{
			name:        "different symbols — no overlap",
			from:        "Context.Bind",
			to:          "validate",
			query:       "Show call chain from Context.Bind to validate",
			wantFrom:    "Context.Bind",
			wantTo:      "validate",
			wantChanged: false,
		},
		{
			name:        "both dot-notation — no overlap",
			from:        "Scene.addMesh",
			to:          "Scene.render",
			query:       "Path from Scene.addMesh to Scene.render",
			wantFrom:    "Scene.addMesh",
			wantTo:      "Scene.render",
			wantChanged: false,
		},
		{
			name:        "to is long enough — not rule 3",
			from:        "read_csv",
			to:          "DataFrame",
			query:       "Show the call chain from read_csv to DataFrame",
			wantFrom:    "read_csv",
			wantTo:      "DataFrame",
			wantChanged: false,
		},
		{
			name:        "empty from — no validation",
			from:        "",
			to:          "validate",
			query:       "Show callers of validate",
			wantFrom:    "",
			wantTo:      "validate",
			wantChanged: false,
		},
		{
			name:        "empty to — no validation",
			from:        "Context.Bind",
			to:          "",
			query:       "Show callees of Context.Bind",
			wantFrom:    "Context.Bind",
			wantTo:      "",
			wantChanged: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotFrom, gotTo, gotChanged := validateExtractedPathParams(tt.from, tt.to, tt.query)
...
// TestReDeriveDestinationFromQuery tests D3b query phrase re-derivation.
func TestReDeriveDestinationFromQuery(t *testing.T) {
	tests := []struct {
		name  string
		query string
		from  string
		...
		{
			name:  "basic two-word concept",
			query: "Show the call chain from Context.Bind to data validation",
			from:  "Context.Bind",
			want:  "dataValidation",
		},
		{
			name:  "two-word concept with question mark",
			query: "Find the path from Engine.ServeHTTP to request handling?",
			from:  "Engine.ServeHTTP",
			want:  "requestHandling",
		},
		{
			name:  "three-word concept",
			query: "Show the call chain from read_csv to DataFrame construction pipeline",
			from:  "read_csv",
			want:  "dataframeConstructionPipeline",
		},
		{
			name:  "single-word concept",
			query: "Show the call chain from main to initialization",
			from:  "main",
			want:  "initialization",
		},
		{
			name:  "no to preposition",
			query: "Show callers of validate",
			from:  "validate",
			want:  "",
		},
		{
			name:  "empty after to",
			query: "Show path from X to",
			from:  "X",
			want:  "",
		},
		{
			name:  "concept with trailing period",
			query: "Find the path from WriteBatch.Set to disk persistence.",
			from:  "WriteBatch.Set",
			want:  "diskPersistence",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := reDeriveDestinationFromQuery(tt.query, tt.from)
			if got != tt.want {
				t.Errorf("reDeriveDestinationFromQuery(%q, %q) = %q, want %q",
					tt.query, tt.from, got, tt.want)
			}
		})
	}
}
