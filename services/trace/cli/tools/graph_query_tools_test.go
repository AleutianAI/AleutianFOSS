...
func TestGetCallChainTool_Execute(t *testing.T) {
	...
	t.Run("resolves class symbol via shared resolution", func(t *testing.T) {
		...
		result, err := classTool.Execute(ctx, map[string]any{
			"function_name": "Scene",
			"direction":     "downstream",
		})
		if err != nil {
	...
	t.Run("resolves dot-notation via shared resolution", func(t *testing.T) {
		...
		result, err := dotTool.Execute(ctx, map[string]any{
...
// TestFindImplementationsTool_AcceptsInterfaceClassStruct tests that interfaces,
// classes, and structs are all accepted as valid targets (IT-03 C-3a), while
// non-type symbols like functions are filtered out.
...
func TestFindImplementationsTool_TextOutputLabels(t *testing.T) {
	...
	t.Run("class target shows Base class label", func(t *testing.T) {
		...
		tool := NewFindImplementationsTool(g, idx)
		result, err := tool.Execute(ctx, map[string]any{"interface_name": "BaseView"})
		if err != nil {
			t.Fatalf("Execute() error = %v", err)
		}
	...
	t.Run("interface target shows Interface label", func(t *testing.T) {
		...
		tool := NewFindImplementationsTool(g, idx)
		result, err := tool.Execute(ctx, map[string]any{"interface_name": "Reader"})
		if err != nil {
			t.Fatalf("Execute() error = %v", err)
		}
...
// =============================================================================
// GR-58: CRS TraceStep Integration Tests for 6 Core Tools
// =============================================================================

// TestFindCallersTool_TraceStepPopulated verifies CRS integration on success path.
...
// =============================================================================
// GR-59 Group A: Definitive Footer Tests
// =============================================================================

// TestFindCallers_DefinitiveFooter verifies the success path includes the definitive answer footer.
...
func TestFindCallees_TypeAliasMessage(t *testing.T) {
	...
	tool := NewFindCalleesTool(g, idx)
	result, err := tool.Execute(ctx, map[string]any{"function_name": "HandlerFunc"})
	if err != nil {
		t.Fatalf("Execute() error: %v", err)
	}
	if !result.Success {
...
func TestFindCallees_ResolvedKindEmpty_WhenNotFound(t *testing.T) {
	...
	tool := NewFindCalleesTool(g, idx)
	result, err := tool.Execute(ctx, map[string]any{"function_name": "NonExistentSymbol"})
	if err != nil {
		t.Fatalf("Execute() error: %v", err)
	}
...
// TestFindImplementations_DefinitiveFooter verifies the success path includes the definitive answer footer.
...
// =============================================================================
// IT-05 CR-2: Inheritance-Aware Upstream Traversal
// =============================================================================

// TestGetCallChainTool_InheritanceAwareUpstream verifies that upstream traversal
// follows inheritance chains to find callers of parent methods.
...
// TestMergeTraversals verifies the traversal merge helper for CR-2.
// IT-00a-1 Phase 4: mergeTraversals now has void return (mutate-only contract).
// Tests verify mutation of primary directly.
...
// TestGetCallChainTool_DestinationParam tests IT-05 R5: destination_name parameter parsing.
...
// TestGetCallChainTool_ExternalBoundaryAnnotation tests IT-05a external boundary
// detection in get_call_chain output. When the traversal reaches a node with
// SymbolKindExternal, the output should annotate it with IsExternal=true and
// include the package in ExternalDependencies.
...
// =============================================================================
// IT-05a: Test fixture helpers
// =============================================================================

// createTestGraphWithExternalUpstream creates a graph where an external node
// is a caller (upstream direction).
