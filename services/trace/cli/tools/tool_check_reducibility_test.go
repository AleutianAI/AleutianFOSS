...
// createTestGraphForReducibility builds a graph with a "main" entry point
// suitable for dominator tree computation and reducibility checking.
func createTestGraphForReducibility(t *testing.T) (*graph.Graph, *index.SymbolIndex) {
	t.Helper()
	...
	// Simple reducible structure: main -> A -> B, main -> C
	...
	g.AddEdge("cmd/main.go:10:main", "pkg/c.go:10:C", graph.EdgeTypeCalls, ast.Location{FilePath: "cmd/main.go", StartLine: 16})

	g.Freeze()
	return g, idx
}

func TestCheckReducibility_CRS_Metadata(t *testing.T) {
	ctx := context.Background()
	g, idx := createTestGraphForReducibility(t)
	...
	requiredKeys := []string{
		"show_irreducible", "is_reducible",
		"irreducible_region_count", "quality_score",
	}
